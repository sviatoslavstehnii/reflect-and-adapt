...
_Generated 2026-03-21. 60 sessions total (3 personas × 2 arms × 10 sessions). Olena pending._
...
All 3 personas show higher personalization_hit for adaptive at holdout. The effect is consistent
...
Marcus: strong quality gain at holdout. Sofia: moderate. Aisha: neutral.
...
Mixed. Only marcus shows a clear overall gain. Sofia and aisha's overall average is dragged down
by early-session noise and metric misalignment (see problems below).
...
Marcus: 4× lower correction rate. Others marginal.
...
Turn efficiency is only meaningful for marcus. Sofia and aisha don't show it — reasons differ
(sofia: sessions have high natural variance; aisha: her inquisitive style saturates turn count
for both arms regardless of adaptation).
...
Sofia late-session: correction rate halved for adaptive, turn efficiency better, phit near-parity
(because baseline gets phit from in-session re-explanation — see confound below).
...
> Cortex consistently encodes user-specific preferences into the workspace across all 3 personas
> (personalization_hit 1.6–3.4× higher at holdout), with the strongest quality gains for users
> whose preferences are structurally encodable. Marcus — whose preferences are binary and
> rule-like (delegate to Andrew, no code, 5-step limit, bullet format) — shows the clearest
> benefit: +0.89 holdout helpfulness, −76% correction rate, and −9% turns per session. Sofia
> shows a holdout quality gain (+0.23) and strong efficiency signal. Aisha shows clear preference
> encoding (3.4× phit) but no helpfulness gain, attributable to metric misalignment with
> learning-oriented interaction styles.
...
## What to do with the 4th persona (olena)

Olena is a data analyst. Her preferences are likely to sit between marcus (structured, tool-specific
requests) and aisha (exploratory, iterative). Key things to watch:

- Does correction rate drop after Cortex encodes her SQL/Python tool preferences?
- Does the holdout show helpfulness gains (closer to marcus pattern) or phit-only gains (aisha pattern)?
- Does the evaluator's conciseness bias affect her sessions the same way it affects aisha?

Olena completing the dataset brings the total to 80 sessions. At that point the holdout comparison
(4 personas × 2 arms) becomes the primary evidence table.
